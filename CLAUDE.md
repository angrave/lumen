...
- Screenshots for the help docs live in `docs/img/`. Re-capture them with playwright after any UI change that affects chat, usage, models, model detail, or projects pages. Use `CONFIG_YAML=./dev.config.yaml uv run python -c "from lumen import create_app; ..."` with a dev config that has `dev_user` set.
...
- DB connection hygiene — narrow rule, don't over-apply: ordinary (non-generator) request handlers should keep relying on Flask-SQLAlchemy's implicit per-request teardown to release `db.session` — that mechanism is correct and well-tested; don't add defensive `db.session.remove()`/commit/rollback calls to plain views that don't need them. The one pattern that genuinely needs an *explicit* release is a **generator that interleaves DB calls with multiple `yield`s and is driven by something other than Flask's own request/response cycle** — e.g. a Prometheus `Collector.collect()` consumed internally by `generate_latest()`, as opposed to a generator returned as the response body via `stream_with_context` (which already has its own established pattern: extract every scalar needed, release the session, *then* start yielding — see `llm.py`'s `send_message_stream` and the streaming paths in `chat/routes.py`/`api/routes.py`). Confirmed in production: `/metrics`' `LumenDBCollector.collect()` was the one call site of the first kind in the codebase, and it leaked a connection idle-in-transaction until Postgres's `idle_in_transaction_session_timeout` killed it — surfacing later as `server closed the connection unexpectedly` on an unrelated request that got handed the dead pooled connection. It now releases its session explicitly in a `finally`; use it as the template for this specific pattern only.
...
This project is indexed by GitNexus as **lumen** (2817 symbols, 6325 relationships, 188 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
...
| Resource | Use for |
|----------|---------|
| `gitnexus://repo/lumen/context` | Codebase overview, check index freshness |
| `gitnexus://repo/lumen/clusters` | All functional areas |
| `gitnexus://repo/lumen/processes` | All execution flows |
| `gitnexus://repo/lumen/process/{name}` | Step-by-step execution trace |
