...
- Added connection-pool leak diagnostics. `lumen_db_pool_connections{state="checked_out"}` climbing and never falling back cannot be attributed to a call site from the gauges alone, so every pool checkout now records its endpoint, thread and stack (`services/pool_tracker.py`), dropped again on check-in. A new `{state="stranded"}` gauge counts checkouts held over 5 minutes — longer than any legitimate call site, since every streaming path releases its connection before the LLM call — and a watchdog logs the outstanding stacks once the pool sits above 90% of capacity for three consecutive scrapes. `GET /metrics/debug` (same bearer token as `/metrics`) returns those outstanding checkouts plus a stack dump of every live thread, which also identifies a WSGI worker thread wedged mid-request.
