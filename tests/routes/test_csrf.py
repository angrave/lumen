...
def test_purge_conversations_rejected_without_csrf_token(csrf_client):
    resp = csrf_client.delete("/profile/conversations")
    assert resp.status_code == HTTPStatus.BAD_REQUEST
    assert b"CSRF" in resp.data


def test_set_store_conversations_rejected_without_csrf_token(csrf_client):
    resp = csrf_client.post(
        "/profile/settings/store-conversations",
        json={"enabled": False},
        content_type="application/json",
    )
    assert resp.status_code == HTTPStatus.BAD_REQUEST
